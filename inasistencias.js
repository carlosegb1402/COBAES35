...
const reportesRefInasistencia = ref(db, 'Inasistencia');



onValue(reportesRefInasistencia, (snapshot) => {
    const container = document.getElementById('cardContainerInasistencia');
    container.innerHTML = ''; 
    ...
    snapshot.forEach((childSnapshot) => {
        ...
        card.innerHTML = `
               <div class="card-header">
                <h3>${value.nombre}</h3>
               <button data-key="${key}" class="delete-button delete-button-inasistencia fa fa-trash"></button>
            </div>
            ...
            <div class="button-container">
                <button data-key="${key}" class="${statusBtnClass} resolver-button-inasistencia">${value.status ? 'No resuelto' : 'Resuelto'}</button>
                <button data-key="${key}" class="printbutton printbuttoninasistencia" onClick="subirScroll()">Imprimir</button>
            </div>
        `;
    ...
    document.querySelectorAll('.delete-button-inasistencia').forEach(button => {
        button.addEventListener('click', async (event) => {
            const key = event.target.getAttribute('data-key');
    
            if (confirm('¿Estás seguro de que deseas eliminar este registro?')) {
                try {
                    const deleteRef = ref(db, 'Inasistencia/' + key);
    ...
    document.querySelectorAll('.resolver-button-inasistencia').forEach(button => {
        button.addEventListener('click', (event) => {
            const key = event.target.getAttribute('data-key');
            const newStatus = event.target.classList.contains('print-button-false') ? false : true;

            const updateRef = ref(db, 'Inasistencia/' + key);
            update(updateRef, {
                status: newStatus
            });
        });
    ...
    document.querySelectorAll('.printbuttoninasistencia').forEach(button => {
        button.addEventListener('click', (event) => {
            const key = event.target.getAttribute('data-key');
            const value = snapshot.child(key).val();
            imprimirInasistencia(value.nombre,value.dia,value.asignaturas,value.horas);
        });
    });
});
