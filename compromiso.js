...
const reportesRefInasistencia = ref(db, 'Carta Compromiso');


onValue(reportesRefInasistencia, (snapshot) => {
    const container = document.getElementById('cardContainerCompromiso');
    container.innerHTML = ''; 
    ...
    snapshot.forEach((childSnapshot) => {
        ...
        card.innerHTML = `
             <div class="card-header">
                <h3>${value.nombre}</h3>
               <button data-key="${key}" class="delete-button delete-button-compromiso fa fa-trash"></button>
            </div>
            ...
            <div class="button-container">
                <button data-key="${key}" class="${statusBtnClass} resolver-button-compromiso">${value.status ? 'No resuelto' : 'Resuelto'}</button>
                <button data-key="${key}" class="printbutton printbuttoncompromiso" onClick="subirScroll()">Imprimir</button>
            </div>
        `;
    ...
    document.querySelectorAll('.delete-button-compromiso').forEach(button => {
        button.addEventListener('click', async (event) => {
            const key = event.target.getAttribute('data-key');
    
            if (confirm('¿Estás seguro de que deseas eliminar este registro?')) {
                try {
                    const deleteRef = ref(db, 'Carta Compromiso/' + key);
    ...
    document.querySelectorAll('.resolver-button-compromiso').forEach(button => {
        button.addEventListener('click', (event) => {
            const key = event.target.getAttribute('data-key');
            const newStatus = event.target.classList.contains('print-button-false') ? false : true;

            const updateRef = ref(db, 'Carta Compromiso/' + key);
            update(updateRef, {
                status: newStatus
            });
        });
    ...
    document.querySelectorAll('.printbuttoncompromiso').forEach(button => {
        button.addEventListener('click', (event) => {
            const key = event.target.getAttribute('data-key');
            const value = snapshot.child(key).val();
            imprimirCarta(value.fecha2,value.nombre,value.grupo,value.dia,value.mes,value.ano);
        });
    });
});
